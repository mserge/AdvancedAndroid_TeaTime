...
public class OrderActivityBasicTest {
    ...
    @Rule  public ActivityTestRule<OrderActivity> mActivityRule =
            new ActivityTestRule(OrderActivity.class);
    // TODO (3) Finish writing this test which will:
    @Test
    public void clickDecrementButton_ChangesQuantityAndCost() {
    //          - Check that the initial quantity is zero
        onView(withId(R.id.quantity_text_view)).check(ViewAssertions.matches(withText("0")));
    ...
    //          - Verify that the decrement button won't decrease the quantity 0 and cost below $0.00
        onView(withId(R.id.quantity_text_view)).check(ViewAssertions.matches(withText("0")));
        onView(withId(R.id.cost_text_view)).check(ViewAssertions.matches(withText("$0.00")));
    }
}